...
def parse_기수명(기수명):
    """
    기수명에서 연도·거래기간을 추출.
    예) '25년 2기 예정' → (2025, '2025.07.01 ~ 2025.09.30')
        '26년 1기 확정' → (2026, '2026.04.01 ~ 2026.06.30')

    분기 규칙:
      1기 예정 → 01.01~03.31 / 1기 확정 → 04.01~06.30
      2기 예정 → 07.01~09.30 / 2기 확정 → 10.01~12.31

    반환: (년도:int|None, 거래기간:str)  — 파싱 실패 시 (None, '')
    """
    s = str(기수명)

    # 연도: 4자리(2025) 우선, 없으면 2자리(25)
    ym = re.search(r'(20\d{2})\s*년', s) or re.search(r'(\d{2})\s*년', s)
    if not ym:
        return None, ''
    yr = ym.group(1)
    ...
    # 기수: 1기/2기
    gm = re.search(r'([12])\s*기', s)
    # 예정/확정
    예정 = '예정' in s
    확정 = '확정' in s
    if not gm or not (예정 or 확정):
        return year, ''   # 연도만이라도 반환
